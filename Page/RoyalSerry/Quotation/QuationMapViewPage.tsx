...
const coordinates= [
  ...
  export class QuationMapViewModel extends BaseViewModel {
    coordinate: any[]=[];
    coordinates: map[]=[];
    currentLocation: any;
  ...
  export default class QuationMapViewPage extends BaseComponent<
  any,
  QuationMapViewModel
> {
  constructor(props) {
    super(props);
    var model = new QuationMapViewModel();
    ...
    return (
          ...
          />
       {
            model.coordinates.map((marker, index) => (
              ...
              >
                <Callout>
                  <Text style={{height:50}}>{marker?.name}</Text>
                </Callout>
